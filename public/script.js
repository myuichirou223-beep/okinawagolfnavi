...
function filterPastTournaments(searchInput) {
  const panel = searchInput.closest("[data-tournament-panel='past']");
  if (!panel) return;
  ...
  const queryParts = searchInput.value
    ...
    .split(/\s+/)
    .filter(Boolean);
  const cards = [...panel.querySelectorAll(".tournament-board")];
  let visibleCount = 0;
